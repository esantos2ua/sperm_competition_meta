...
#super[b] Departamento de Zoologia, Instituto de Biociências, Universidade de São Paulo, São Paulo, SP,
...
and evidence on testis investment and sperm quality in ART species was conflicting
 @smithRyanEvolutionSpermQuality2010 @simmonsSpermCompetitionGamesDimorphic1999 @kellySpermInvestmentRelation2008 @snookSpermCompetitionNot2005 .

Del Matto  @delmattoSpermCompetitionGames2018  provided the first cross-species meta-analytic test of both
predictions, in fishes. From searches of *Web of Knowledge_ and _Scopus_ last updated _16 May 2017* (868 +
...
$I^2$ = 83.21%). By variable type, *minors invested substantially more in GSI (−2.638, −3.105 to −2.177)*,
...
substituted a *species-level five-level rank_ and fitted it as an _unordered factor* interacted with
...
niger_ only, 15 effect sizes; SCR 5 = _Axoclinus nigricaudus* only, 3 effect sizes, all from Neat
...
- *Q1c (behavioral sperm allocation per mating):* Del Matto reported that *majors allocate significantly
  ...
  *between-morph comparisons_ from _within-male experimental manipulations of perceived risk*, (ii) separating
...
#align(center)[
  ...
  #table(
    ...
    [*Scope & Taxa*],
    [Fishes only (50 studies, 29 species, 183 effect sizes)],
    [All animals (92 studies, 67 species; 58 fish studies)],
    [Reconciled fishes master set + post-2020 literature update + non-fish extension],
    [*Study Overlap & Scope*],
    [Archived 50 fish studies (1995–2017)],
    [Shared 31 fish studies; 27 fish studies unique to broad search (19 fish studies unique to Del Matto)],
    [Full synthesis uniting both scopes (31 shared + 19 fish Del Matto + 27 fish Dougherty + post-2020)],
    [*Behavioral Allocation*],
    [Formal category ($k = 11, g = +2.732$; majors allocate more per spawn)],
    [Not evaluated (focused on standardized static ejaculate metrics across phyla)],
    [Core functional axis testing ejaculate economy during mating interactions],
    [*Testes & GSI Allometry*],
    [GSI only ($k = 31, g = -2.638$); 24 absolute mass rows excluded],
    [Focused on absolute testis mass to avoid ratio scaling artifacts],
    [Bivariate meta-regression modeling body mass & dimorphism as continuous allometric covariates],
    [*Sperm Competition Proxy*],
    [5-level categorical SCR (Stockley 1997); extreme ranks 1 & 5 confounded],
    [Sneaker frequency (linear test); evaluated cross-taxa gradient],
    [Continuous quadratic polynomial testing Parker's intermediate peak + continuous frequency],
    [*Phylogeny*],
    [Vector art tree without branch lengths; dropped from final models],
    [Open Tree of Life synthetic tree (`rotl`)],
    [Calibrated Ray-finned fish tree (Rabosky / `fishtree`) with Grafen branch lengths],
    [*Variance & Provenance*],
    [Single total $N$ per row; sampling variances not archived],
    [Re-extracted per-morph statistics for included subset],
    [Complete re-extraction with exact variances via `metafor::escalc`; fully open data],
    [*Overall Goal*],
    [Original MSc dissertation (unpublished)],
    [Landmark broad-taxa animal benchmark (*Biol Rev*)],
    [Definitive, reproducible publication resolving allometry & non-linear risk],
  )
]
...
- *"Minor" is not a homogeneous category across species.* Sneakers, satellites, female mimics and
  ...
  can anticipate competition  @taborskyEvolutionAlternativeReproductive2008 @kustraAlonzoSpermAlternativeReproductive2023 . Q1d treats this as a moderator rather than noise, and
...
2. *Confirm the effect-size count reconciliation in code.* The Results text states *183 effect sizes from 50
   studies and 29 fish species*. The table yields *50 sources_ and _29 species* (the latter after treating
   *Parablennius parvicornis_ and _Parablennius sanguinolentus parvicornis* as one species; 29 also equals the
   ...
   *107 are Quality (matching exactly)*, *11 are Allocation (matching exactly)* and *89 are Production
   ...
   split *31 GSI + 34 Quantity*, with Quality *107_ and Allocation _11* unchanged; the dissertation's
...
4. *Reconstruct sampling variances.* Supplementary Table 1 reports $g$ and a *single* $N$ per row with no
   ...
   $v = (n_1 + n_2)/(n_1 n_2) + g^2 / (2(n_1 + n_2))$ with $n_1 = n_2 = N/2$
...
6. *Re-fit and compare.* Re-implement the three models in R with `metafor::rma.mv`
   ...
   (−0.699, 0.197)*; the $I^2$ decompositions; and Egger intercepts.
...
questions*, above). Under the PECOS framework  @richardsonWellbuiltClinicalQuestion1995 @fooPracticalGuideQuestion2021 , our main eligibility criteria are:
...
search strings will be *translated_ and the results screened by _reviewers with expertise in the
...
We will also: (i) *complete the original screening_ by retrieving and screening the _14 title/abstract-
...
species (backward/forward snowballing;  @taborskyEvolutionAlternativeReproductive2008 @kustraAlonzoSpermAlternativeReproductive2023 @lupoldHowSpermCompetition2020 @kellyJennionsSexualSelectionSperm2011 @buzattoAlternativePhenotypesWithin2014 ); (iii) consult the datasets
of published comparative analyses of fish sperm traits  @stockleySpermCompetitionFishes1997 @fitzpatrickFemalePromiscuityPromotes2009 ; and (iv) *contact authors* for per-morph means, dispersions and
...
search of all *Web of Knowledge_ databases and the advanced search of _Scopus*, last updated *16 May 2017*:
...
#note[
*Note on the keyword set.* The original string is reproducible but has three weaknesses we will address rather than inherit. (i) The *tactic block contains no plain synonyms* for the phenotypes themselves — *sneak\*, _satellite_, *parasitic spawn\*, *bourgeois*, *jack*, *female mimic*, *dwarf male*, *territorial* — so studies that describe the tactics without using the phrase "alternative … tactic" can be missed. (ii) The *outcome block is narrow*: it omits *GSI*, *gonadosomatic*, *testes mass*, *milt*, *sperm velocity*, *sperm motility*, *sperm number*, *sperm quality*, *spermatocrit*, *seminal vesicle*, and *paternity*. (iii) *Leading wildcards* (`"male_ _morph*"`) and mid-phrase wildcards (`"post*copulato*"`, `"behavio*r*"`) are handled inconsistently across platforms and may have behaved differently in *Web of Knowledge_ and _Scopus*. For the update we will construct explicit, reproducible Boolean strings per database from an expanded tactic block crossed with an expanded ejaculate-trait block, add a taxon block for the Q1e extension search (insects, amphibians, reptiles, birds, mammals), then check relative recall against the benchmark set.
]
...
We will conduct *two-stage screening_ in _Rayyan*  @ouzzaniRayyanWebMobile2016 : (1)
...
#align(center)[
  ...
  #table(
    ...
    [identifierExtractor],
    [Initials of the data extractor],
    [Restricted; ESAS / LADM / …],
    [identifierStudyId],
    [First author + year + source],
    [Free text; Neat_2001_EnvBiolFish],
    [identifierStudyYear],
    [Publication year (online if differs from print)],
    [Numeric (4 digits); 2001],
    [identifierStudyDoi],
    [DOI of the study],
    [Free text; 10.1023/A:1011095717581],
    [identifierEffectSizeId],
    [Unique row identifier (observation level)],
    [Free text; ES_0001],
    [dataProvenance],
    [New extraction vs. carried over from the 2018 appendix],
    [Restricted; New / Archived / New+Archived (matched)],
    [inOriginalDataset],
    [Whether this contrast is represented in Del Matto (2018)],
    [Restricted; Yes / No],
    [inDougherty2022],
    [Whether this contrast is represented in Dougherty et al. (2022)],
    [Restricted; Yes / No],
    [identifierDoughertyStudyId],
    [Study identifier in Dougherty et al. (2022) dataset],
    [Free text; Dougherty_045 or NA],
    [taxonomySpecies],
    [Study species (accepted binomial)],
    [Free text; Axoclinus nigricaudus],
    [taxonomySpeciesOriginal],
    [Species name as printed in the source / 2018 appendix],
    [Free text; Parablennius sanguinolentus parvicornis],
    [taxonomyFamily / taxonomyOrder],
    [Family and order],
    [Free text; Tripterygiidae / Blenniiformes],
    [taxonomyGroup],
    [Broad taxon for the Q1e extension],
    [Restricted; Fish / Insect / Amphibian / Reptile / Bird / Mammal / Other],
    [experimentalPopulation],
    [Population / site studied],
    [Free text],
    [experimentalSetting],
    [Where the data were collected (needed to reproduce model 2's subset)],
    [Restricted; Field-observational / Field-experimental / Laboratory],
    [experimentalDesign],
    [Nature of the contrast],
    [Restricted; Between-morph / Within-male risk manipulation],
    [tacticMajorLabel],
    [Verbatim term the study uses for the major tactic],
    [Free text; "territorial", "hooknose", "nest-holder"],
    [tacticMinorLabel],
    [Verbatim term the study uses for the minor tactic],
    [Free text; "sneaker", "satellite", "jack", "female mimic"],
    [tacticMinorType],
    [Coded minor tactic type (Q1d)],
    [Restricted; Sneaker / Satellite / Female mimic / Other / Mixed],
    [tacticPlasticity],
    [Tactic architecture (Q1d)],
    [Restricted; Fixed-genetic / Plastic-conditional / Sequential / Unknown],
    [tacticNumberDescribed],
    [Number of male tactics described in the species],
    [Numeric; 2 / 3 / …],
    [moderatorFertilizationMode],
    [Fertilization mode (Q1d; the assumption the original invokes)],
    [Restricted; External / Internal / Buccal],
    [moderatorSpermCompetitionRank],
    [Sperm competition rank, re-coded by two coders],
    [Restricted; 0 / 1 / 2 / 3 / 4 / 5],
    [moderatorSCROriginal],
    [Rank as assigned in Del Matto (2018), for comparison],
    [Restricted; 1–5 or NA],
    [moderatorSCRSource],
    [Source of the life-history data used to assign the rank],
    [Free text; FishBase / primary reference],
    [moderatorMinorFrequency],
    [Proportion of minor males in the population, if reported (Q1a)],
    [Numeric (0–1) or NA],
    [moderatorMinorFrequencySource],
    [Where the frequency came from],
    [Restricted; Same study / Other study, same population / Other population / NA],
    [responseVariableOriginal],
    [Response variable exactly as named in the source],
    [Free text; "GSI (energy based)"],
    [responseCategory],
    [Sperm expenditure category (2018 scheme)],
    [Restricted; Production / Quality / Allocation],
    [responseSubcategory],
    [Production sub-category (absent from the 2018 appendix; re-derived)],
    [Restricted; GSI / Quantity / NA],
    [responseIndexType],
    [Whether the trait is an absolute mass or a body-size-corrected index (Q1b)],
    [Restricted; Absolute mass / Relative index / Not a mass],
    [redundancyGroup],
    [Links absolute and relative versions of the same organ measured on the same males],
    [Free text; RG_012 or NA],
    [responseTimePoint],
    [Time of measurement post sperm activation, where relevant],
    [Free text; "first measurement after activation"],
    [majorMean / minorMean],
    [Per-morph means],
    [Numeric],
    [majorSD / minorSD],
    [Per-morph dispersion, converted to SD if SE/CI reported],
    [Numeric],
    [majorN / minorN],
    [Per-morph sample sizes (number of males)],
    [Numeric],
    [totalN],
    [Total $N$ as reported (the only $N$ archived in the 2018 appendix)],
    [Numeric],
    [dispersionReported],
    [What the source actually reported],
    [Restricted; SD / SE / 95% CI / None],
    [inferentialStatistic],
    [Test statistic used when descriptive statistics were unavailable],
    [Free text; t = 2.31, df = 45],
    [bodyMassMajor / bodyMassMinor],
    [Per-morph body mass (for the allometric analysis, Q1b)],
    [Numeric (g; unit recorded) or NA],
    [bodyMassDimorphism],
    [Ratio of major to minor body mass],
    [Numeric or NA],
    [effectSizeG],
    [Hedges' $g$ (positive = larger in majors)],
    [Numeric],
    [effectSizeVariance],
    [Sampling variance of $g$],
    [Numeric],
    [effectSizeGArchived],
    [$g$ as printed in the 2018 Supplementary Table 1, for cross-checking],
    [Numeric or NA],
    [effectSizeGDougherty],
    [$g$ as reported in Dougherty et al. (2022) dataset, for cross-checking],
    [Numeric or NA],
    [effectSizeSourceType],
    [Where the value was extracted from],
    [Restricted; Text / Table / Figure / Supplementary / Author-supplied],
    [effectSizeProvenance],
    [How the analysis effect size was obtained (for flagging in sensitivity analyses)],
    [Restricted; Directly calculated / From inferential statistics / Converted between metrics / Reconstructed from 2018 appendix],
    [sharedControlId],
    [Identifier for contrasts sharing a control or the same males],
    [Free text; SC_007 or NA],
    [excludedIn2018],
    [Whether this contrast was excluded from the 2018 analysis (e.g. absolute gonad mass)],
    [Restricted; Yes / No / Unknown],
    [descriptionStudyComplexity],
    [Complexity of extraction (proxy for effort/expertise)],
    [Restricted; Easy / Moderate / Hard],
    [descriptionGeneralNote],
    [Relevant extraction notes, incl. verbatim tactic wording and any discrepancy with the 2018 appendix],
    [Free text],
  )
]
...
SCR on the observational, non-allocation subset, study + species) — using `metafor::rma.mv`
 @viechtbauerConductingMetaanalysesMetafor2010 , the modified multilevel $I^2$
...
sample sizes  @hedgesDistributionTheoryGlass1981 @borensteinIntroductionMetaanalysis2009 @viechtbauerConductingMetaanalysesMetafor2010 , retaining the original's sign convention (positive = majors
...
(observation) ID_ and _species* — the observation-level term the original did not describe — plus a
...
and *partial $I^2$ per random factor* [`orchaRd::i2_ml`; @nakagawaOrchaRd20Package2023;
@yangPluralisticFrameworkMeasuring2025; benchmarked against typical ecological values,
@seniorHeterogeneityEcologicalEvolutionary2016], and results visualised with *orchard_ and _bubble* plots
...
*a priori* moderators are: *index type_ and _body mass* (Q1b), *experimental design* and
*fertilization mode* (Q1c, Q1d), *tactic plasticity_ and _minor tactic type* (Q1d), and *taxonomic
group* (Q1e).
...
explicitly: for fishes, from the ray-finned fish tree of life
 @raboskyInverseLatitudinalGradient2018 @changFishtreeRPackage2019 , falling
...
comparability; (ii) as an *ordered/continuous_ moderator with a _linear and quadratic term* within each
...
*omnibus test of the moderator* and $R^2$-analogues, which the original did not. Every rank-level estimate
...
minimum: (1) *leave-one-study-out_ and _leave-one-species-out* re-fits (recomputing the VCV matrix and
